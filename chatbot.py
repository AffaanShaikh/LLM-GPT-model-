...
block_size = 12 #block size/ sequence length #ensure block_size > max_new_tokens
...
class GPTLanguageModel(nn.Module): #GPT - Generative Pre-trained Transformer
    ...
    def generate(self, index, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            #crop index to the last block_size tokens to crop around max_new_tokens limit
            index_cond = index[:, -block_size:] #index condition
            #get the predictions
...
#.unsqueeze to upack the tensors in matrices form from [[],[]] to [],[]
while True:
    prompt = input("Prompt:\n") 
